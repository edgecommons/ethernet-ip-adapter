...
| connection gone | the `enip` `Closed` error, mapped by `map_enip_error` | the I/O manager no longer holds the connection, so the buffer would never ride a frame |
