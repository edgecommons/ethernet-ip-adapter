...
2. [Decisions register (D-EIP-1…D-EIP-25)](#2-decisions-register)
...
**Instance routing (D-EIP-13 + D-EIP-25):** every verb accepts `"instance": "<id>"` in the body,
and every verb is registered via `register_scoped` so an instance-addressed delivery topic
(`…/{instance}/cmd/{verb}`, SOUTHBOUND §2.2) routes too. The topic token is authoritative: a
conflicting `body.instance` is `BAD_ARGS`; topic-only routes by the token. On a component-scoped
delivery, `body.instance` is optional iff exactly one device is configured (then it defaults to
that device); with ≥ 2 devices a missing instance is `BAD_ARGS` and an unknown one
`NO_SUCH_INSTANCE`. Replies always include the resolved `"id": "<instance>"`.
...
5. **CIP Security — Phase 1 (TLS on the explicit path) shipped; DTLS/commissioning are the
   roadmap.** Poll (explicit-messaging) instances run over TLS (EtherNet/IP over TLS, TCP 2221,
