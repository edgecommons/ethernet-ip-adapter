...
2. [Decisions register (D-EIP-1…D-EIP-33)](#2-decisions-register)
...
- **Arrays** (`arrayCount: N`): poll reads request N elements (`read_tag(&tag, N)`); push fields
  ...
  **N is bounded to `1..=65535`** — the CIP Read Tag element count is a `u16` — and the bound is a
  ...
  must keep**: the poll path compares the elements a read returned against the elements it asked
  for, and a mismatch is a BAD sample naming expected vs got, not a short array published GOOD. A
...
- **`bool` arrays are EXPERIMENTAL** in poll mode (D-EIP-16). `{"type": "bool", "arrayCount": N}` is
  accepted and encoded byte-per-element (`0xC1`), and each such signal raises one validation WARN:
  the encoding is best-effort and unvalidated against physical hardware, Logix controllers pack BOOL
  arrays as DWORDs (1756-PM020) so a ControlLogix/CompactLogix read is expected to report BAD, and
  behavior on other devices depends on their implementation — the adapter has no device-family
...
| `config.rs` | precedence chain (signal ▸ group ▸ device ▸ global ▸ built-in); duplicate name/tagPath rejection; `string` type rejection; unknown-key rejection (template `deny_unknown_fields` kept); allow-list parse; slot range; mode/block cross-validation (`poll`+`io` rejected, `push`+`pollGroups` rejected); push layout validation (field beyond `sizeBytes`, `bit` on non-bool, output signals on a heartbeat connection — all rejected at startup); the D-EIP-33 cardinality bounds — `arrayCount` `0`/`70000`/`u32::MAX` rejected naming the bound while `1` and `65535` validate, and the refusal proven to operate at **device** granularity like every other signal-level rejection; the D-EIP-16 BOOL-array warning as a **value** rather than a log scrape — `bool` + `arrayCount` accepted and addressable, exactly one warning per offending signal (a scalar `bool` and a non-bool array are silent), the wording pinned on its `&'static str` constant (experimental / byte-per-element / not validated against physical hardware / DWORD / 1756-PM020) **and** pinned NOT to claim a device family works, plus the bounds still applying to a bool array |
...
2. **No structures/UDTs, no strings; BOOL arrays are experimental.** Supported values are CIP
   ...
   encoding is byte-per-element and unvalidated against physical hardware, and because Logix packs
   BOOL arrays as DWORDs (1756-PM020) such a read from a ControlLogix/CompactLogix reports BAD —
   with the reason in the sample's `qualityRaw`. Behavior on other devices depends on their
   implementation; the adapter has no device-family detection, so the experimental label covers the
   feature as a whole. Scalar `bool` and push assembly `bool` array fields are unaffected. Array
...
6. **Lab-hardware-only validation paths, declared:** the `slot`/backplane routing path,
   ...
   **Logix BOOL-array (DWORD) packing** — a `BOOL[n]` tag is served as a `0x00D3` DWORD array
   (1756-PM020), and the layout has to be captured off a real controller before it can be decoded:
   cpppo is a Logix emulator with a fidelity gap at exactly this point (it serves BOOL arrays
   byte-per-element), so a packing model written against the bench would be validated by the very
   thing that hides the mismatch, and the feature cannot be validated here at all. Until then the
   adapter's byte-per-element BOOL-array encoding ships **labelled experimental**, warned per signal
   at validation and self-explaining in the BAD sample it produces against a controller
   (D-EIP-16) —
