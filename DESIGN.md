...
`--locked`, and `coverage` additionally front-loads a bare `cargo metadata --locked` so a lock that
...
with no committed lock. `fuzz-smoke` and `live-sims` are `pull_request`-only, and the workflow's `paths-ignore`
