...
- **`bool` arrays are EXPERIMENTAL** in poll mode (D-EIP-16, D-EIP-35). `{"type": "bool",
  ...
  unpacked LSB-first — element *n* is bit *(n mod 32)* of word *(n / 32)* — into exactly N booleans,
  published in the same JSON shape either representation would produce (an array of N, or the bare
  value when N is 1, matching the one-element collapse above). N words: `ceil(N/32)`, with bits past N in the final word dropped as device
...
|---|---|---|---|
| `device-connected` | emit, Info | session established | `{instance, adapter, endpoint}`, plus `identity` (the D-EIP-34 nameplate object, same shape as `sb/status.identity`) when the device answered the Identity Object read — absent, never `null`, when it did not |
...
{ "id": "filler-plc", "mode": "poll", "connected": true, "state": "ONLINE", "paused": false,
  ...
  "identity": { "vendorId": 1, "vendorName": "Rockwell Automation/Allen-Bradley",
                "deviceType": 14, "deviceTypeName": "Programmable Logic Controller",
                "productCode": 55, "revision": "20.11", "serialNumber": "0x1234ABCD",
                "productName": "1756-L71/B" },
  "dialect": { "tagListService": "unknown" } }
```

**Identity (D-EIP-34).** `identity` is always present and is `null` when the session is down or the
device refused the Identity Object read — a refusal is an ordinary outcome, not an error state.
`vendorName`/`deviceTypeName` are `null` for codes outside the protocol stack's known-values table;
the numeric field beside each is the authority. `serialNumber` is the zero-padded hex label, as a
string. The value is captured at connect and answered from memory, so a status query costs no device
round-trip. It is **informational**: nothing in the adapter dispatches on it, because the device
asserts it and nothing proves it.

**Dialect memory (D-EIP-34).** `dialect` is always present. `tagListService` is `supported` once a
browse has been answered by the device's Logix tag-list service, `unsupported` once the device has
refused that service (`BROWSE_UNSUPPORTED`), and `unknown` until an operation has settled it — a
`BROWSE_FAILED` (a link error mid-walk) teaches nothing and leaves it as it was. Nothing is probed
for at connect; the memory records what operations that ran anyway have taught.
