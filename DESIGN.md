...
session handle → translator task → the `enip` I/O manager's `stage_output` ack → back — bounded by
`timeouts.requestTimeoutMs`, so `ok:true` asserts that the I/O manager holds this buffer for a
**live** class-1 connection. Staging that cannot be confirmed is an inline `ok:false` naming the
reason, with the usual `write-audit` Warning, never a success:
