...
| **D-ENIP-26** | **Two identity reads, one per layer, and a pre-registration request sender.** (1) `EipClient::read_identity()` reads the **CIP Identity Object** (class `0x01`, instance 1) with a single `Get_Attributes_All` over the established session and decodes the mandatory attribute block (1–7) into `IdentityObject`; trailing bytes — the optional attributes a well-equipped device appends — are **ignored, not refused**, and a block that ends early is `WireError::Truncated`. A device that refuses the service yields `EnipError::Cip`, an outcome the caller is expected to tolerate. (2) `list_identity_over(stream, deadline)` sends the **encapsulation** `ListIdentity` (`0x0063`) over an already-connected, *unregistered* byte stream and decodes the reply into the existing `DeviceIdentity`: one request, one reply, no retry and no discard loop, the whole exchange bounded by one absolute deadline, with the reply required to echo `LIST_IDENTITY_CONTEXT` and the command. The Identity class/instance/attribute constants move to `discovery.rs` and the §7.6 keepalive shares them. | The two reads answer the same question at different layers and neither substitutes for the other: the Identity **Object** needs a session and is what a connected client should ask; **ListIdentity** is defined over an unregistered connection, which is what makes it usable when a target accepts TCP and then refuses to open a session — the one case where nothing else can be asked. `EipClient::identity()` already sent ListIdentity *over a session*; it stays (the poll session's cheapest liveness probe), and the new free function is what a caller with a bare stream needs, because the failed handshake's stream is consumed and dropped inside `connect`. One `Get_Attributes_All` rather than six `Get_Attribute_Single`s: the mandatory block is specified as an ordered concatenation, so one reply carries everything, and the caller pays this on every reconnect. Tolerating trailing bytes is load-bearing, not lenience — the cpppo bench peer appends four bytes past the product name, and refusing them would make the identity of a conforming device unreadable. Requiring the context echo is what makes the pre-registration exchange correlated at all: before the session actor exists there is no monotonic context, and any `ListIdentity`-shaped frame already on the stream could otherwise be adopted as the peer's answer (the same reasoning as `REGISTER_CONTEXT`, §5.5). |
...
`EipClient::identity()` asks for it over an established session. `list_identity_over(stream,
deadline)` asks for it over a connected but **unregistered** stream (D-ENIP-26) — the form that
still works against a target which accepts TCP and then refuses to open a session. It writes one
request stamped with `LIST_IDENTITY_CONTEXT`, reads one reply, and requires that reply to echo the
context and the command; the whole exchange is bounded by the caller's absolute deadline.
...
**The Identity Object read (D-ENIP-26).** `EipClient::read_identity()` is the one typed reading
of that escape hatch the crate ships: a single `Get_Attributes_All` against `IDENTITY_CLASS` /
`IDENTITY_INSTANCE`, decoded by `IdentityObject::parse_get_attributes_all` into vendor, device
type, product code, revision, status, serial and product name — the mandatory attributes 1–7, in
attribute order. Anything after the product name is an optional attribute whose presence is
device-specific, so trailing bytes are ignored; a block that ends early is `WireError::Truncated`.
A device that refuses the service returns `EnipError::Cip`, which is a legitimate answer: the
libplctag `ab_server` peer refuses every Identity service it is offered.
...
UDP as bare CPF frames (no encapsulation header): O→T to the **target's** registered implicit-I/O
port **2222** (`0x08AE`), T→O back to whatever port the originator bound and advertised in the
ForwardOpen's Sockaddr Info items (§8.2) — an **ephemeral** one here, so the originator never
contends for 2222 with a target on the same host. The TCP session
...
Bare CPF on the class-1 UDP flow (§8.1) — **no encapsulation header**:
...
- Peer-driven counters (`stale_frames`, `malformed_frames`, `overflowed_events`,
  ...
  second interface, which otherwise presents only as a link that never comes up. The adapter maps it
  through its seam onto the `sourceMismatchDatagrams` measure and the push `sb/status` `io` object
  (DESIGN §8.8/§7.1), so the drop is visible to an operator and not only to `stats()`.
...
let nameplate = client.read_identity().await?;                             // Identity Object 0x01/1 (D-ENIP-26)
client.close().await;

// ---- implicit (push) ----
// An ORIGINATOR binds an ephemeral source port and advertises it in the ForwardOpen's Sockaddr Info
// items (§8.2), so it never contends for :2222 with a target on the same host. Only a device acting
// in the TARGET role binds the registered implicit-I/O port `IO_UDP_PORT` (2222) to be found there.
let io = IoManager::bind("0.0.0.0:0").await?;
...
**Output staging comes in three forms** (D-ENIP-20). `set_output` is synchronous and *unconfirmed*:
...
plainly that the value was never staged. `stage_output_by` is the same call under an absolute
deadline, which it hands to the socket task along with the buffer: the handoff and the verdict are
bounded by it, and a command that outlives it is **dropped at the producer buffer rather than
staged**, so `Err(Timeout { op: "output staging" })` means the value cannot appear on a later frame.
Callers answering a write command under their own budget use `stage_output_by`; the fire-and-forget
setter stays for callers with nothing to report to.
...
| `fuzz_discovery` | discovery reply decoders (§5.3): `DeviceIdentity::parse_reply` / `parse_item` (sockaddr item + SHORT_STRING product-name length games), the CIP Identity Object attribute-block parser `IdentityObject::parse_get_attributes_all` (§7.5, D-ENIP-26), and the `parse_list_services` / `parse_list_interfaces` CPF walkers |
