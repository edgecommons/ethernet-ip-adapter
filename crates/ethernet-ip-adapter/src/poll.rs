...
pub(crate) async fn poll_until_disconnected(
    ...
    loop {
        // Earliest wake: the nearest group deadline, batch-window close, and health emit.
        let mut wake = *deadlines.iter().min().expect("at least one poll group");
        if let Some(bd) = engine.next_batch_deadline(batch_ms) {
            wake = wake.min(bd);
        }
        wake = wake.min(since_health + metrics_interval);
